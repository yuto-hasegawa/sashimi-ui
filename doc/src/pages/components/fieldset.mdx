...
import FieldsetUsage from "../../components/usages/fieldset";

# Fieldset

`.fieldset` is for html fieldset element.
...
React component `<Fieldset/>` is available.  
It is used to wrap other form components, <a href={`${import.meta.env.BASE_URL}components/input/`}>`Input`</a>, <a href={`${import.meta.env.BASE_URL}components/textarea/`}>`Textarea`</a>,  <a href={`${import.meta.env.BASE_URL}components/select/`}>`Select`</a>, <a href={`${import.meta.env.BASE_URL}components/label/`}>`Label`</a>, <a href={`${import.meta.env.BASE_URL}components/legend/`}>`Legend`</a>, <a href={`${import.meta.env.BASE_URL}components/helper-text/`}>`HelperText`</a>.  
**`<Fieldset/>` passes `error` status to its children**, just like html `fieldset` does its `disabled` status.

<SamplePane filled>
<FieldsetUsage client:idle />
</SamplePane>
...
export default function FieldsetUsage() {
  ...
  return (
    <form className="card" style={{ display: "flex", flexDirection: "column", gap: 32, width: 480 }}>
      ...
      <Fieldset error={message.length <= 10 ? undefined : "message is too long"}>
      ...
      <Fieldset error={neta === "pudding" ? "pudding is not a neta" : undefined}>
      ...
      <Fieldset className="fieldset" error={drink === "milk" ? "error" : undefined}>
